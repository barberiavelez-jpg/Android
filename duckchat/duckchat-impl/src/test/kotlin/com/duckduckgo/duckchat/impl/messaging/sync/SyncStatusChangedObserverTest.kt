...
class SyncStatusChangedObserverTest {
    ...
    private val mockDuckChatFeature: DuckChatFeature = mock()
    ...
    fun setUp() {
        ...
        whenever(mockDuckChatFeature.supportsSyncStatusSubscription()).thenReturn(mockToggle)
    ...
    @Test
    fun `when feature flag is disabled then no events are emitted`() = runTest {
        ...
        observer.syncStatusChangedEvents.test {
            ...
            // Sign in - but feature disabled, no emission
            syncStateFlow.emit(SyncState.READY)
            expectNoEvents()

            // Sign out - but feature disabled, no emission
            syncStateFlow.emit(SyncState.OFF)
            expectNoEvents()
        }
    }
